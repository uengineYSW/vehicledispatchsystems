<template>
    <div style="max-height:80vh;">
        ...
        <div class="gs-bundle-of-buttons" style="max-height:10vh;">
            ...
            <v-btn @click="updateDriverDialog = true" class="contrast-primary-text" small color="primary" :disabled="!hasRole('Administrator')">
                <v-icon small>mdi-minus-circle-outline</v-icon>운전자 정보 업데이트
            </v-btn>
            <v-dialog v-model="updateDriverDialog" width="500">
                <UpdateDriverCommand
                    @closeDialog="updateDriverDialog = false"
        ...
        <wj-flex-grid
            ...
            class="wj-felx-grid"
        >
            <wj-flex-grid-filter :filterColumns="['RowHeader','affiliation','contactNumber','isAvailable',]" />
            <wj-flex-grid-cell-template cellType="RowHeader" v-slot="cell">{{cell.row.index + 1}}</wj-flex-grid-cell-template>
            <wj-flex-grid-column binding="affiliation" header="소속" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="contactNumber" header="연락처" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="isAvailable" header="사용여부" width="2*" :isReadOnly="true" align="center" />
        </wj-flex-grid>
        <v-col>
            <v-dialog
                v-model="openDialog"
                ...
                <template>
                    <v-card>
                        <v-toolbar
                            ...
                            height="50px"
                        >
                            <div style="color:white; font-size:17px; font-weight:700;">운전자 등록</div>
                            <v-spacer></v-spacer>
                            <v-icon
                                color="white"
                                small
...
export default {
    ...
    methods:{
        updateDriver(params){
            try{
                this.repository.invoke(this.getSelectedItem(), "updateDriver", params)
                this.$EventBus.$emit('show-success','UpdateDriver 성공적으로 처리되었습니다.')
                this.updateDriverDialog = false
            }catch(e){
